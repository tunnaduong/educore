<div class="p-4">
        ...
        @php
            ...
            foreach ($currentRounds as $round) {
                \Log::info('Round in view: ID=' . $round->id . ', Name=' . $round->name . ', Start=' . $round->start_date . ', End=' . $round->end_date);
            }
            ...
            if ($student && $currentRounds->count() > 0) {
                ...
                foreach ($currentRounds as $round) {
                    ...
                    \Log::info('Student ' . $student->id . ' evaluated round ' . $round->id . ' in view: ' . ($evaluated ? 'YES' : 'NO'));
                ...
                @else
                    <form wire:submit.prevent="saveEvaluation">
                        ...
                        <div class="card shadow-sm mb-4">
                            <div class="card-header bg-primary text-white">
                                <h5 class="mb-0">
                                    ...
                                    Nhóm 1: Đánh giá về giáo viên
                            <span class="badge bg-light text-primary ms-2">{{ count($teacherQuestions) }} câu hỏi</span>
                                </h5>
                            </div>
                            <div class="card-body">
                                @foreach ($teacherQuestions as $key => $question)
                                    ...
                                    <div class="star-rating" data-question="teacher_{{ $key }}">
                                            ...
                                            <input type="radio" class="star-input"
                                                           ...
                                                           value="{{ $i }}"
                                                           id="teacher_{{ $key }}_{{ $i }}">
                                            <label class="star-label" for="teacher_{{ $key }}_{{ $i }}" data-rating="{{ $i }}">
                        ...
                        <div class="card shadow-sm mb-4">
                            <div class="card-header bg-success text-white">
                                <h5 class="mb-0">
                                    ...
                                    Nhóm 2: Đánh giá về chất lượng khóa học
                            <span class="badge bg-light text-success ms-2">{{ count($courseQuestions) }} câu hỏi</span>
                                </h5>
                            </div>
                            <div class="card-body">
                                @foreach ($courseQuestions as $key => $question)
                                    ...
                                    <div class="star-rating" data-question="course_{{ $key }}">
                                            ...
                                            <input type="radio" class="star-input"
                                                           ...
                                                           value="{{ $i }}"
                                                           id="course_{{ $key }}_{{ $i }}">
                                            <label class="star-label" for="course_{{ $key }}_{{ $i }}" data-rating="{{ $i }}">
                                                <i class="bi bi-star"></i>
                                                    </label>
                                                @endfor
                                            </div>
                        ...
                        <div class="card shadow-sm mb-4">
                            <div class="card-header bg-warning text-dark">
                                <h5 class="mb-0">
                                    ...
                                    Nhóm 3: Cảm nhận cá nhân
                            <span class="badge bg-light text-warning ms-2">{{ count($personalQuestions) }} câu hỏi</span>
                                </h5>
                            </div>
                            <div class="card-body">
                        @foreach ($personalQuestions as $key => $question)
                                <div class="mb-4">
                                    ...
                                    <div class="star-rating" data-question="personal_{{ $key }}">
                                            ...
                                            <label class="star-label" for="personal_{{ $key }}_{{ $i }}" data-rating="{{ $i }}">
                                                <i class="bi bi-star"></i>
                                                </label>
                                            @endfor
                                        </div>
                        ...
                        @endforeach
                                ...
                                {{ count($teacherQuestions) + count($courseQuestions) + count($personalQuestions) + 1 }}. Bạn có đề xuất gì để cải thiện chất lượng giảng dạy hoặc môn học không?
                                    </label>
