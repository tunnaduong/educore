...
<div>
                    <a href="{{ route('lessons.create') }}" wire:navigate class="btn btn-success"><i class="bi bi-plus-circle me-1"></i> Thêm bài học</a>
                </div>
            </div>
        </div>
        <!-- Filters -->
                            ...
                            <tbody>
                                @foreach ($lessons as $lesson)
                                    <tr>
                                        ...
                                        <td>
                                            @if($lesson->video)
                                                <a href="{{ $lesson->video }}" target="_blank" class="badge bg-warning text-dark">Video</a>
                                            @else
                                        ...
                                        <td>
                                            @if($lesson->attachment)
                                                <a href="{{ asset('storage/' . $lesson->attachment) }}" target="_blank" class="badge bg-success">Tài liệu</a>
                                            @else
                                                <span class="text-muted">-</span>
                                            @endif
                                        </td>
                                        ...
                                        <td>
                                            <a href="{{ route('lessons.show', $lesson->id) }}" wire:navigate class="btn btn-sm btn-outline-primary"><i class="bi bi-eye"></i></a>
                                            <a href="{{ route('lessons.edit', $lesson->id) }}" wire:navigate class="btn btn-sm btn-outline-warning"><i class="bi bi-pencil"></i></a>
                                            <button type="button" class="btn btn-sm btn-outline-danger" wire:click="confirmDelete({{ $lesson->id }}, '{{ addslashes($lesson->title) }}')"><i class="bi bi-trash"></i></button>
                                        </td>
                                    </tr>
                                @endforeach
                            </tbody>
        ...
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <p>Bạn có chắc chắn muốn xoá bài học "<strong>{{ $lessonTitleToDelete }}</strong>"?<br>Hành động này không thể hoàn tác.</p>
                </div>
