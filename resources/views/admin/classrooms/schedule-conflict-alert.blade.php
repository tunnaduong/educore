                ...
                <div class="flex-grow-1">
                    ...
                    <div class="d-flex justify-content-between align-items-center mt-3">
                        <button type="button" class="btn btn-sm text-muted" wire:click="toggleConflicts">
                            ...
                            {{ $showConflicts ? __('general.hide_details') : __('general.view_details') }}
                        </button>
                        @if ($showConflicts && count($conflicts) > 3)
                            <small class="text-muted">
                                <i class="bi bi-info-circle mr-1"></i>
                                @lang('general.conflicting_students_count', ['count' => count($conflicts)]) @lang('general.scroll_to_view_all')
                            </small>
                        @endif
                    </div>
                </div>
